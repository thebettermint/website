...
const icons = fs.readdirSync(ICON_SOURCE_FOLDER)
  .filter(isSVG)
  .map(removeExtension);
  ...
  icons
    .map((icon:any) =>
        `export const ${toPascalCase(
          icon,
        )} = (props:any) => <Icon {...props} name="${icon}" />;`,
    )
    .join('\n'),
].join('\n');
...
fs.writeFileSync(`../svg/icon-map.tsx`, iconMapContent);